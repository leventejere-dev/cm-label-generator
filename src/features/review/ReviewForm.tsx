...
export function ReviewForm({ data, removed, onChange }: ReviewFormProps) {
  ...
  const handleStandardChange = (group: FieldGroupId, key: string, value: string) => {
    ...
    setError(id, verdict.ok ? null : verdict.message ?? 'This value cannot be used.');
    markEdited(id);
  ...
  const handleAdditionalLabel = (index: number, label: string) => {
    ...
    setError(id, verdict.ok ? null : verdict.message ?? 'This field name cannot be used.');
  ...
  return (
    <div className="stack">
      {lowConfidenceCount > 0 ? (
        <Card padded>
          <strong>Please verify the highlighted fields before generating the label.</strong>
          <p className="muted" style={{ marginTop: 4, fontSize: 'var(--cm-text-sm)' }}>
            {lowConfidenceCount} value{lowConfidenceCount === 1 ? '' : 's'} could not be read with
            high confidence. Compare them with the photo.
          </p>
        </Card>
      ) : null}
      ...
      {FIELD_GROUPS.map((group) => (
        <GroupCard
          ...
          onAdditionalAdd={(label, targetGroup) => {
            const verdict = checkCaption(label);
            if (!verdict.ok) {
              setError('new-field', verdict.message ?? 'This field name cannot be used.');
              return false;
            }
      ...
      <div className="group-card__header">
        ...
        <span style={{ marginLeft: 'auto', textTransform: 'none', letterSpacing: 0 }}>
          <Badge tone={group === 'delivery' ? 'accent' : 'neutral'}>
            {group === 'delivery' ? 'Your details' : populated.length + extras.length}
          </Badge>
        </span>
      </div>
  ...
  return (
    <div style={{ display: 'flex', gap: 'var(--cm-space-2)' }}>
      <select
        ...
        value={value}
        aria-label="Add a standard field"
...
function AdditionalOnlyCard({
  ...
  return (
    <Card padded={false} flush>
      <div className="group-card__header">Additional information</div>
      <div className="group-card__body">
        {extras.length === 0 ? (
          <p className="muted" style={{ padding: 'var(--cm-space-4) 0', fontSize: 'var(--cm-text-sm)' }}>
            Anything the supplier printed that does not fit a standard field appears here.
          </p>
        ) : null}
